...
double ComputeError(const double *grid, const double *new_grid, size_t n) {
    double error = 0.0;
#pragma acc parallel loop collapse(2) reduction(max:error) present(grid, new_grid)
    for (size_t y = 1; y < n - 1; ++y) {
        for (size_t x = 1; x < n - 1; ++x) {
            size_t idx = y * n + x;
            error = fmax(error, fabs(grid[idx] - new_grid[idx]));
        }
    }
    return error;
}
...
int main(int argc, char **argv) {
    ...
    for (size_t i = 0; i < n_max_iterations && error > epsilon; ++i) {
        ...
        if (i % 1000 == 0) {
            error = ComputeError(grid, new_grid, n);
        }
        nvtxRangePop();
